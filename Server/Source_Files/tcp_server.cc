...
int main() {
	...
	while (true) {
		cout << "\x1b[32m" << "クライアントの接続を待機しています" << "\x1b[0m" << endl;
		...
		thread t1(thread_main1, client_sock, user, cref(thread_id2));
		thread t2(thread_main2, client_sock, user, cref(thread_id1));
...
void thread_main1(const int& client_sock, const string& user, const pthread_t& thread_id) {
	while (true) {
		try {
			const Data data = parse_data_json(receive_from(client_sock, '\0'));

			if (data.method == "post")
				get_message(client_sock, data.message);
	...
	while (true) {
		...
		try {
			if (command != "exit") {
				...
				post_message(client_sock, message);
			}
			else {
				quit(client_sock, user);
